...
?></title>
    ...
    <link rel="stylesheet" type="text/css" media="all" href="/themes/<?php echo $sTheme; ?>/css/cheddarFront.css?t=<?php echo time(); ?>" />
    <link rel="stylesheet" type="text/css" media="all" href="/themes/<?php echo $sTheme; ?>/css/jquery/ui-lightness/jquery-ui-1.8.16.custom.css" />
    ...
    if (!empty($oPage->style_sheets)) {
      foreach ($oPage->style_sheets as $file) {
        ?>
        <link rel="stylesheet" type="text/css" media="all" href="<?php echo $file; ?>?t=<?php echo time();?>" />
        <?php
      }
    }
    ?>
    ...
    if (!empty($oModule->css)) {
      foreach (explode(' ', $oModule->css) as $file) {
        ?>
  ...
  fjs.parentNode.insertBefore(js, fjs);
              ...
              <?php if (!empty($user_id)) {?>
                <li>
                  <div id="user-info-wrapper">
                    <a href="/profile"><?php print $user_info->screen_name ?></a><br />
    ...
    <div class="container">
      <div class="content">
        ...
        <div class="wrapper">
          <div class="<?php echo (empty($sSubMenu) ? 'proper-content-no-secondary' : 'proper-content') ?>">
            <?php
            //hack for shoping cart
            if($oPage->url =='store/product/*') $oPage->layout_id = 98;
            if($oPage->url =='shop' && $oPage->title == 'Shopping Cart') $oPage->layout_id = 99;
...
<?php }else{         
            ...
            <?php } elseif ($oPage->layout_id == 3) { ?>
              <!--two column layout right side bigger -->
              <div class="container-fluid" id="mainContent">
                <div class="row-fluid">
                  <div class="span4">
                    <?php
                        ...
                        foreach ($oPage->sections as $section) {
                          if ($section->zone == 'left_zone') {
                            if (empty($section->module_widget)) {
                              ...
                              //var_dump($section->content_widget);
                              if (isset($section->content_widget) && !empty($section->content_widget)){
                                print $section->content_widget;
                              }
                            }
                          }
            ...
            <?php } elseif ($oPage->layout_id == 5 || $oPage->layout_id == 98)  { ?>
              <!-- two 100% content boxes
              -->
              <div class="container-fluid" id="mainContent">
                <div class="row-fluid">
                  <div class="span12">
                    <div class="hero-unit">
                      <?php
                      $module_view_used = false;
                      //upper_zone in template
                      foreach ($oPage->sections as $section) {
                        if ($section->zone == 'upper_zone') {
                          if (empty($section->module_widget)) {
                            print $section->content;
                          } else {
                            if (isset($section->content_widget))
            ...
            <?php } elseif ($oPage->layout_id == 10) { ?>
              <!-- 100% content box 1 column-->
              <div class="container-fluid" id="mainContent">
                <div class="row-fluid">
                  <div class="span12">
                    <div class="hero-unit">
    ...
    <div class="vertical-menu visible-mobile hidden">
      ...
      <div class="menu-items">
        <ul class="unstyled">
          <?=$sMobileMenu?>
          <?php if (empty($user_id)) { ?>
          <li data-parentid="-1"><a href="http://<?php echo $sMainDomain?>/login">Log In</a></li>
