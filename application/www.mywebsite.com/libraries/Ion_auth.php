...
* Modified by: jeffrey@hottomali.com
* Update summary:
*   added the capability of having multiple roles per user
...
class Ion_auth
{
	...
	public function forgotten_password($email)
	{
		if ( $this->ci->ion_auth_model->forgotten_password($email) )
		{
			...
			$this->ci->postmark->initialize($config);
			//$this->ci->postmark->set_newline("\r\n");
			$this->ci->postmark->from($this->ci->config->item('admin_email', 'ion_auth'), $this->ci->config->item('site_title', 'ion_auth'));
			$this->ci->postmark->to($user->email);
			$this->ci->postmark->subject($this->ci->config->item('site_title', 'ion_auth') . ' - Forgotten Password Verification');
			$this->ci->postmark->message_html($message);

			if ($this->ci->postmark->send())
			{
    ...
    if (!is_object($profile))
    {
		...
		if ($new_password)
		{
			...
			//$this->ci->postmark->set_newline("\r\n");
			$this->ci->postmark->from($this->ci->config->item('admin_email', 'ion_auth'), $this->ci->config->item('site_title', 'ion_auth'));
			$this->ci->postmark->to($profile->email);
			$this->ci->postmark->subject($this->ci->config->item('site_title', 'ion_auth') . ' - New Password');
			$this->ci->postmark->message_html($message);
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $additional_data, $site_id = 1, $group_name = false)
	{
	  $email_activation = $this->ci->config->item('email_activation', 'ion_auth');

		if (!$email_activation)
		{
			if ($this->ci->ion_auth_model->register($username, $password, $email, $additional_data, $site_id, $group_name))
			{
		...
		{
			$id = $this->ci->ion_auth_model->register($username, $password, $email, $additional_data, $site_id, $group_name);
			...
			$message_html = $this->ci->load->view($this->ci->config->item('email_templates', 'ion_auth').$this->ci->config->item('email_activate', 'ion_auth'), $data, true);
			$this->ci->postmark->clear();
			$config['mailtype'] = "html";
			...
			//$this->ci->postmark->set_newline("\r\n");
			$this->ci->postmark->from($this->ci->config->item('admin_email', 'ion_auth'), $this->ci->config->item('site_title', 'ion_auth'));
			$this->ci->postmark->to($email);
			$this->ci->postmark->subject($this->ci->config->item('site_title', 'ion_auth') . ' - Account Verification');
			$this->ci->postmark->message_html($message_html);
			

			if ($this->ci->postmark->send() == TRUE)
			{
    ...
    $this->ci->session->unset_userdata('siteID');
	...
	{
    return has_permission('admin_area');
