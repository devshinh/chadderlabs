...
define('CS_REST_WRAPPER_VERSION', '1.0.10');
...
class Cmonitor {
    ...
    function put_request($route, $data, $call_options = array()) {
        return $this->_call($call_options, CS_REST_PUT, $this->_base_route.$route, $data);
    }

    function post_request($route, $data, $call_options = array()) {
        return $this->_call($call_options, CS_REST_POST, $this->_base_route.$route, $data);
    }

    function delete_request($route, $call_options = array()) {
        return $this->_call($call_options, CS_REST_DELETE, $this->_base_route.$route);
    }

    function get_request($route, $call_options = array()) {
        return $this->_call($call_options, CS_REST_GET, $this->_base_route.$route);
    }
    ...
    function get_request_paged($route, $page_number, $page_size, $order_field, $order_direction,
        ...
        return $this->get_request($this->_base_route.$route);
    }
    ...
    function _call($call_options, $method, $route, $data = NULL) {
        ...
        return new __construct($call_result['response'], $call_result['code']);
    }
}
