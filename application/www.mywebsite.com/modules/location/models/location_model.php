...
class Location_model extends HotCMS_Model {
  ...
  /**
   * get_location_by_slug() - get oraganization for DB by location id
   *
   *
  ...
  public function get_location_by_slug($slug) {
    ...
    $this->db->where($this->tables['location'] . '.slug', $slug);
    $query = $this->db->get($this->tables['location']);
    return $query->row();
  }
  ...
  /**
   *  set_coordinates() - set ltitude and longtitude
   *
   *
   *  @param id location
   *  @return object with one row
  ...
  */
  ...
  /**
   * List locations by user ID
   * @param  int  $user_id
   * @return mixed
   */
  public function list_locations($user_id = 0)
  {
    if ($user_id > 0) {
      $this->db->where('ul.user_id', $user_id);
    }
    $this->db->select('l.*')
      ->join($this->tables['location'] . ' l', 'l.id=ul.location_id');
    $query = $this->db->get($this->tables['location_user'] . ' ul');
    return $query->result();
  }
