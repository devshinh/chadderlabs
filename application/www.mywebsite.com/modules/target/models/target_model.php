...
// haven't yet implemented keyword/search function.
  ...
  function get_stores($categories = FALSE, $types = FALSE, $and = TRUE, $org_id = 0) {
    ...
    foreach ($organizations as $organization) {
      $organization_locations = $this->db->select("l.*, s.province_name")->distinct()->join($this->tables["state"]." s", "s.province_code=l.province")->order_by("l.store_name", "asc")->get_where($this->tables["store"]." l", array("l.retailer_id" => $organization->id))->result();
  ...
  function get_targets_by_site($site_id = 1, $get_detail = FALSE, $org_id = 0) {
    ...
    if (is_object_array($targets) && $get_detail) {
      foreach ($targets as $key => $target) {
        ...
        $organizations = $this->db->select("o.*")->join($this->tables["organization"]." o", "o.id=t.organization_id")->where("o.id !=", 130)->where("o.id !=", 131)->get_where($this->tables["target_organization"]." t", array("t.target_id" => $target->id))->result();
        ...
        $types = $this->db->select("t.*")->join($this->tables["organization_type"]." t", "t.id=tt.organization_type_id")->get_where($this->tables["target_organization_type"]." tt", array("tt.target_id" => $target->id))->result();
        if (is_array($types) && ( !empty($types))) {
    ...
    if (is_object_array($targets)) {
      foreach ($targets as $target) {
        ...
        if ( !(isset($target->organizations) OR isset($target->locations)) && (isset($target->categories) OR isset($target->types))) {
         ...
         $targeted_organizations = $this->get_organizations($category_ids, $type_ids);
          if (is_object_array($targeted_organizations)) {
             $organizations = array_objects_merge($organizations, $targeted_organizations);
          }
        }
      }
    }
    ...
    if (is_object_array($targets)) {
      foreach ($targets as $target) {
        ...
        if ( !(isset($target->organizations) OR isset($target->locations)) && (isset($target->categories) OR isset($target->types))) {
          ...
          $targeted_locations = $this->get_stores($category_ids, $type_ids, TRUE, $org_id);
          if (is_object_array($targeted_locations)) {
             $locations = array_objects_merge($locations, $targeted_locations);
          }
        }
      }
  ...
  function get_targeted_members($site_id = 1, $targets = FALSE, $locations = FALSE) {
    ...
    if (is_object_array($locations) ) {
      $location_ids = array();
      $organzation_ids = array();
      foreach ($locations as $location) {
        $location_ids[$location->id] = $location->id;
        $organzation_ids[$location->retailer_id] = $location->retailer_id;
      }
      if ( !empty($location_ids)) {
        $members = $this->db->select("u.created_on, up.user_id, up.screen_name, up.retailer_id, up.store_id")->distinct()->where_in("up.store_id", $location_ids)->where_in("up.retailer_id", $organzation_ids)->join($this->tables["user_profile"]." up", "up.user_id = u.id")->join($this->tables["user_role"]." ur", "ur.user_id = u.id")->join($this->tables["role"]." r", "r.id = ur.role_id")->order_by("up.screen_name", "asc")->get_where($this->tables["user"]." u", array("r.system" => 3))->result();
