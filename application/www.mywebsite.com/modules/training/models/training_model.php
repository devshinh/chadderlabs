...
class Training_model extends HotCMS_Model {
  ...
  public $comm_props = array('category_id', 'title', 'slug', 'status', 'editor_id', 'featured', 'featured_image_id', 'description',
  ...
  public function get_random_slug() {
    $training_ids = $this->session->userdata("trainings");
    if (empty($training_ids)) {
      return FALSE;
    } else {
      $training_ids = explode(",", $training_ids);
    }
    $query = $this->db->select('slug')
            ->where('status', 1)
            ->where('site_id', $this->site_id)
            ->where_in("id", $training_ids)
            ->order_by('', 'random')
            ->limit(1)
  ...
  public function get_preview_slug($type) {
    if ( !$this->account_model->is_super_admin($this->session->userdata("user_id"))) {
      ...
      if (empty($training_ids)) {
        ...
        $this->db->where_in("id", explode(",", $training_ids));
      }
    }
    $this->db->select('slug')
            ->where('status', 1)
   ...
   */
  public function training_load($id = 0, $slug = '', $live_only = TRUE, $current_site_only = TRUE, $targeting_user = TRUE)
  {
    if ($targeting_user && !$this->account_model->is_super_admin($this->session->userdata("user_id"))) {
      ...
      if (empty($training_ids)) {
        ...
        $training_ids = explode(",", $training_ids);
        $this->db->where_in("t.id", $training_ids);
      }
    }
    $id = (int) $id;
    $slug = trim($slug);
   ...
   * @return array of objects
   */
  public function training_list($category_id = 0, $live_only = TRUE, $page_num = 1, $per_page = 0)
  {
    ...
    //$query = $this->db->select('t.*')
      ->join($this->tables['user'] . ' u', 'u.id=t.editor_id')
      ->join($this->tables['category'] . ' c', 'c.id=t.category_id')
   ...
   * @return int
   */
  public function training_count($category_id = 0, $live_only = TRUE) {
    //if ($category_id > 0) {
    //  $this->db->where('category_id', (int)$category_id);
    //}
    if ($live_only) {
      $this->db->where('status', 1);
      $table = $this->tables['training'];
    } else {
      $table = $this->tables['revision'];
    }
    $query = $this->db->where('site_id', $this->site_id)
            ->get($table);
    return $query->num_rows();
...
//            ->get($this->tables['variant_detail']);
  ...
  /**
   * Get a site's product training page url
   * @param  int    $site_id row id in site table
   * @return string found url
   */
  function get_site_training_url($site_id = 0) {
    if ($site_id > 1) {
      $query = $this->db->where("site_id", $site_id)->like("url_parser", "training")->get($this->tables["page"]);
      if ($query->num_rows() > 0) {
