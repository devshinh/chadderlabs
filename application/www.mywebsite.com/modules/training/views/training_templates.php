<div>
  ...
  <div class="row">
    <form action="/hotcms/<?php echo $module_url ?>/template_reload" method="post" name="category">
      ...
      <?php
      $selected_training_category = $selected_category;
      $add = 'onChange="document.category.submit()"';
      echo form_dropdown("training_category", $categories_dropdown, $selected_training_category, $add);
      ?>
    </form>
  </div>
  <div class="leftColumn">
    <h3>Training Item Tags</h3>
    <?php
                ...
                for ($i = 0; $i < count($s); $i++) {
                  if ($type->id == $s[$i]->type_id) {
                    ...
                    <tr>
                      ...
                      printf('<td>%s</td><td>%s</td><td class="last">%s</td>', $s[$i]->name, $edit_link, $delete_link);
                      ?>
                    </tr>
                    <?php
                  }
  ...
  <div class="rightColumn">
    ...
    if (!empty($variant_fields)) {
      foreach ($variant_fields as $field) {
        ...
        echo '<div class="variant_type row editable" id="field_id_' . $field->id . '">';
