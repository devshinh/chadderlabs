        ...
        elseif ($section->section_type == 1 && $section->module_widget > '') {
          ...
          if (!empty($widget_output)) {
            ...
            if (is_string($widget_output)) {
              $section->content_widget = $widget_output;
            } else {
            ...
            //css from config files 
            if(!empty($widget_output['css'])){
                $style_files =  explode(' ',$widget_output['css']);
                foreach($style_files as $file){
                    $module_css = '/modules/' . $method_parts[0] . '/css/' . $file;
                    $filename = FCPATH . APPPATH . $module_css;
                    if (!in_array($module_css, $this->style_sheets) && file_exists($filename)) {
                      $this->style_sheets[] = $module_css;
                    }
            ...
            //css from config files 
            if(!empty($widget_output['js'])){
                $style_files =  explode(' ',$widget_output['js']);
                foreach($style_files as $file){
                    $module_js = '/modules/' . $method_parts[0] . '/js/' . $file;
                    $filename = FCPATH . APPPATH . $module_js;
                    if (!in_array($module_css, $this->javascripts) && file_exists($filename)) {
                      $this->javascripts[] = $module_js;
                    }
                }
            }           
