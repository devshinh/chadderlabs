...
class Asset_item {
      ...
      switch ($this->type) {
        ...
        case 3:   // video
          ...
          if ($source > '') {
            ...
            $this->thumb_html = sprintf('<img src="%s%s%s%s" alt="%s" class="asset_selector" width="183" />',
            ...
            $this->lightbox_html = sprintf('<video id="video_%d" style="display:none" class="sublime lightbox" controls preload="none"
              width="%d" height="%d" %s data-setup="{}">%s</video>', $this->id, $this->width, $this->height, $poster, $source);
          }
          else {
  ...
  public static function list_all_items($type = 0, $category_id = 0, $page_num = 1, $per_page = 1000)
  {
    $CI =& get_instance();
    $rows = $CI->model_asset->get_all_assets($type, $category_id, $page_num, $per_page);
    $assets = array();
    foreach ($rows as $row) {
      $asset = new self($row);
      $assets[$row->id] = $asset;
  ...
  {
    return self::list_all_items(1, $category_id, $page_num, $per_page);
  }

  public static function count_all_items($type = 0, $category_id = 0)
  {
    $CI =& get_instance();
    return $CI->model_asset->count_all_assets($type, $category_id);
  }
