<table class="editinline">
  ...
  if (is_array($images) && count($images) > 0) {
    foreach($images as $image) {
      echo '<tr id="assetitem_' . $image->id . '" class="media_library_item"><td class="imageids">';
      echo form_hidden('id[' . $image->id . ']', $image->id);
      printf('<span class="asset_thumb"><img src="http://%s%s/%s.%s" alt="%s" /></span></td>', $this->session->userdata( 'siteURL' ), $image->thumbnail, $image->file_name.'_thumb', $image->extension, $image->description);
