...
if (!function_exists('load_activity_items')) {
   ...
   function load_activity_items($activity_list){
        ...
        $is_login = ($CI->session->userdata("user_id") !== FALSE); // if no such data, session function returns FALSE; in this case, it means current viewer is not log in
        foreach ($activity_list as $item) {
            ...
            if ($item->point_type == 'quiz') {
                ...
                if ($quiz_history) {
                    $quiz = new CmsQuiz($quiz_history->quiz_id, FALSE, FALSE, $is_login); // only target check for the logged in user
                    $item->quiz_type_name = new stdClass();
                    $item->quiz_type_name = $quiz->type->name;
                    $training = new CmsTraining($quiz->training_id, FALSE, FALSE, $is_login);
            ...
            elseif ($item->point_type == 'quiz-draw') {
                ...
                if (!empty($quiz_history)) {
                    $quiz = new CmsQuiz($quiz_history->quiz_id, FALSE, FALSE, $is_login);
                    $item->quiz_type_name = new stdClass();
                    $item->quiz_type_name = $quiz->type->name;
                    $training = new CmsTraining($quiz->training_id, FALSE, FALSE, $is_login);
                    $item->training_domain = new stdClass();
...
//var_dump($item);die();
            ...
            }elseif ($item->point_type == 'upload_avat') {
                $avatar = $CI->user_model->get_user_avatar($item->avatar_id);
                if(!empty($avatar)){
                  $item->avatar = $avatar;
                }            }
            ...
            $item->screen_name = sprintf('<a href="http://www.cheddarlabs.com/public-profile/%s">%s</a>',  strtolower($item->screen_name),$item->screen_name);
        }
