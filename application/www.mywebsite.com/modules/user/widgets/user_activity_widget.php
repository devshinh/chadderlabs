        ...
        foreach ($activity_list as $item) {
            ...
            if ($item->point_type == 'quiz') {
                ...
                if (!empty($quiz_history)) {
                    $quiz = new CmsQuiz($quiz_history->quiz_id, FALSE, FALSE, FALSE);
                    $item->quiz_type_name = new stdClass();
                    $item->quiz_type_name = $quiz->type->name;
                    $training = new CmsTraining($quiz->training_id, FALSE, FALSE, FALSE);                 
...
//            }                
        ...
        // load widget view    
        if($data['userid']!=0){
            if (!empty($activity_list) OR $restricted) {
                $data['items'] = $activity_list;
                return array(
                    'meta_subtitle' => 'User Activity',
                    'content' => $this->render('user_activity', $data),
                );
            }
        }else{
                return array(
