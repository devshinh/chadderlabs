...
class News_model extends HotCMS_Model {
   ...
   */
  public function get_preview_slug($type, $item_id = 0) {
    $this->db->select('slug')
            ->where('status', 1)
            ->where('site_id', $this->site_id);
    ...
    if ($type == 'training_item') {
      $this->db->join($this->tables['news_item'] . ' i', 'i.news_id=n.id');
      if ($item_id != 0) {
        $this->db->where('i.training_item_id', $item_id);
      }
      $this->db->order_by('create_timestamp', 'ASC');
    }
    if ($type == 0) {
    ...
    $query = $this->db->get($this->tables['news'] .' n');         
    if ($query->num_rows > 0) {
      $result = $query->result();
    } else {
      $result = '';
   ...
   * @return array of objects
   */
  public function list_all_news($category = 0, $live_only = TRUE, $user_id = 0, $page_num = 1, $per_page = 100)
  {
    ...
    $query = $this->db->select('n.*,u.username')
      ...
      ->order_by('n.create_timestamp', 'DESC')
