...
class Product_model extends HotCMS_Model {
   ...
   * @return array of objects
   */
  public function list_products($category_id = 0, $live_only = TRUE, $page_num = 1, $per_page = 100)
  {
    ...
    if ($live_only) {
      ...
      //$table = $this->tables['product_live'] . ' p';
    //} else {
      $table = $this->tables['product'] . ' p';
    }
  ...
  public function get_product($id = 0, $slug = '', $live_only = TRUE)
  {
    ...
    $item = $query->row();
    if ($item->featured_image_id > 0) {
      $this->load->helper('asset/asset');
      $item->featured_image = asset_load_item($item->featured_image_id);
    }
    else {
  ...
  private function _setElement()
  {
    ...
    $this->db->set( 'minimum_bid', $this->input->post( 'minimum_bid' ) );
    $this->db->set( 'minimum_increment', $this->input->post( 'minimum_increment' ) );
