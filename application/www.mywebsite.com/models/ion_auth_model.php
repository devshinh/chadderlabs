...
class Ion_auth_model extends CI_Model
{
  ...
  /**
   * request changing email, send out a
   *
   * @return bool
   * @author Jeffrey
   **/
  public function change_email_request($id, $email)
  {
    if (empty($id))
    {
      return FALSE;
    ...
    if ( $this->db->affected_rows() == 1){
      ...
      $message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_verify', 'ion_auth'), $data, true);
   ...
   * @author Jeffrey
   **/
  public function change_email($id, $code)
  {
    $query = $this->db->select('new_email')
      ->where('activation_code', $code)
      ->limit(1)
      ->get($this->tables['users']);
    $result = $query->row();
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $additional_data = false, $site_id = 1, $group_name = false)
	{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$this->db->select($this->identity_column.', id, password, email')
			->where($this->identity_column, $identity);
    ...
    $query = $this->db->where('active', 1)
        ...
        //$this->session->set_userdata('siteID', $result->site_id );
