<div class="module">
    ...
    if ($filters['sort_by'] == 'name') {
        ...
        ?>"><?php echo lang('hotcms_organization') ?></th>
                        <th id="sortable_active_store_count" class="sortable<?php
                        if ($filters['sort_by'] == 'active_store_count') {
                            echo ($filters['sort_direction'] == 'desc' ? ' headerSortDown' : ' headerSortUp');
                        }
        ?>"><?php echo lang('hotcms_number_of_active_locations') ?></th>
                        <th id="sortable_pending_store_count" class="sortable<?php
                        if ($filters['sort_by'] == 'pending_store_count') {
                            echo ($filters['sort_direction'] == 'desc' ? ' headerSortDown' : ' headerSortUp');
                        }
        ?>"><?php echo lang('hotcms_number_of_pending_locations') ?></th>   
                        ...
                        <th class="filter">
                            <div class="filter_icon<?php if ($filters['country'] != '') echo ' active'; ?>" id="country"></div>
                        </th>
                        ...
                        <th class="filter">
                          <div class="filter_icon<?php if ($filters['status'] != '') echo ' active'; ?>" id="status"></div>
                        </th>        
                ...
                <tbody><pre>
                    ...
                    foreach ($retailers as $row) { 
                        ...
                        <tr id="trData_<?php echo $row->id ?>">
                            ...
                            <td><?php
                echo $row->active_store_count;
                if ($row->active_store_count > 0) {
                    printf('&nbsp;(<a href="/hotcms/%s/store/%d">view</a>)', $module_url, $row->id);
                }
                        ?></td>
                            <td><?php
                echo $row->pending_store_count;
                if ($row->pending_store_count > 0) {
                    printf('&nbsp;(<a href="/hotcms/%s/store/%d">view</a>)', $module_url, $row->id);
                }
                        ?></td>
                            <td><?php echo $row->users; ?>
                <?php if ($row->users > 0) {
                    printf('&nbsp;(<a href="/hotcms/%s/users/%d?filter=clear">view</a>)', $module_url, $row->id);
                }?> </td>
