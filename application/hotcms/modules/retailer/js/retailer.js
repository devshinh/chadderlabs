jQuery( document ).ready( function() {
  ...
  */
  ...
  /*  jQuery("input.ret_per").change(function() {
        var selector = jQuery(this);
        var permission_key = selector.attr('value');
        var retailer_id = selector.attr('style');
        var checked = selector.attr('checked');
        if (checked ==='checked'){
            //add permission   
            var ajax_url = "/hotcms/ajax/retailer/access_add/" + retailer_id + "/" + permission_key + "/" + Math.random()*99999;
            jQuery.getJSON(ajax_url, function(json) {
              if (json.result) {
                selector.after('&nbsp; <img src="/hotcms/asset/images/icons/accept.png" width="21" height="16" alt="updated" style="vertical-align:middle" />');
              }
            ...
            //remove permission
            var ajax_url = "/hotcms/ajax/retailer/access_del/" + retailer_id + "/" + permission_key + "/" + Math.random()*99999;
            jQuery.getJSON(ajax_url, function(json) {
              if (json.result) {
                selector.after('&nbsp; <img src="/hotcms/asset/images/icons/cancel.png" width="21" height="16" alt="updated" style="vertical-align:middle" />');
              }
            }).error(function(){ alert("Sorry but there was an error."); });
            return false;          
        }
