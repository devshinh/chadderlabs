...
<?php
    foreach ($items as $item) {
      if(array_key_exists($item->id, $items_images)){
        ...
        $front_image = 'image place holder'; //sprintf('<img src="/asset/upload/image/tmb_200/%s.%s" alt="%s"/>',$items_images[$item->id]->file_name,$items_images[$item->id]->extension, $items_images[$item->id]->name);
        ?>
