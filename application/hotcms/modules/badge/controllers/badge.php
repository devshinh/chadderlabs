...
class Badge extends HotCMS_Controller {
  ...
  public function edit($id, $page_num = 1)
  {
    ...
    $data['form']['feed_description_input'] = $this->_create_text_input('feed_description', $badge->activity_feed_description, 250, 20, 'text');
    ...
    $data['form']['award_amount_input'] = $this->_create_text_input('award_amount', $badge->award_amount, 50, 20, 'text');
  ...
  public function ajax_update_big_image($badge_id, $asset_id) {
    ...
    if ($id > 0) {
      ...
      try {
        ...
        $messages = 'Badge hover image added.';
      } catch (Exception $e) {
        $messages = 'There was an error when trying to update image: ' . $e->getMessage();
      }
    } else {
