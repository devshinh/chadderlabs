...
class CmsQuiz {
   ...
   * @return void
   */
  public function __construct($identifier = NULL, $live_only = FALSE, $current_site_only = TRUE)
  {
    ...
    if (!empty($identifier)) {
      ...
      $this->load($live_only, $current_site_only);
    }
  }
   ...
   * @return void
   */
  public function load($live_only = TRUE, $current_site_only = TRUE)
  {
    if ($this->id < 1 && $this->slug == '') {
      return FALSE;
    }
    $row = $this->ci->quiz_model->quiz_load($this->id, $this->slug, $live_only, $current_site_only);
    if ($row) {
  ...
  public function save($attr = array())
  {
    ...
    if ($result) {
      $this->set_message('Quiz updated successfully. Please refresh the page to verify.');
    }
    return $result;
  }
  ...
  public function quiz_finish($quiz_history_id, $user_id, $answers)
  {
    ...
    elseif (has_permission('earn_points')) {
      ...
      var_dump($ce_before_deduction);
      var_dump($ce_previous);
      var_dump($ce_earned);
      die('a');
      if ($ce_earned > 0) {
        $this->ci->load->helper('account/account');
        ...
        $result = account_add_contest_entries($history->user_id, $ce_earned, 'quiz', $this->tables['history'], $history->id, $description);
        if (!$result) {
          $this->set_error('Sorry but there was an error when trying to grant points.');
        }
      ...
      $points_before_deduction = 0;
      $points_earned = 0;
      // TODO: determine how many draw entries a user can get each time after they finish the same quiz
      $draw_entry = 1;
    }
    $history_data = array(
  ...
  public function quiz_history_check_attempts($user_id)
  {
    ...
    if ($this->type->tries_per_day > 0) {
      ...
      if ($attempted >= $this->type->tries_per_day) {
        $this->set_error('Sorry but you can only take this quiz ' . $this->type->tries_per_day . ' times per day.');
        return FALSE;
      }
    }
    ...
    if ($this->type->tries_per_week > 0) {
      ...
      if ($attempted >= $this->type->tries_per_week) {
        $this->set_error('Sorry but you can only take this quiz ' . $this->type->tries_per_week . ' times per week.');
        return FALSE;
      }
    }
    return TRUE;
