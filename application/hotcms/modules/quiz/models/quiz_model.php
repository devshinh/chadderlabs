...
class Quiz_model extends HotCMS_Model {
   ...
   */
  public function quiz_load($id = 0, $slug = '', $live_only = TRUE, $current_site_only = TRUE)
  {
    $id = (int)$id;
    $slug = trim($slug);
    if ($id == 0 && $slug == '') {
    ...
    $query = $this->db->get($this->tables['quiz'] . ' q');
    return $query->row();
  }
  ...
  public function quiz_insert($attr)
  {
    ...
    $this->db->set('target_id', array_key_exists('target_id', $attr) ? $attr['target_id'] : 0);
  ...
  public function quiz_update($id, $attr)
  {
    ...
    if (is_array($attr)) {
      if (array_key_exists('quiz_type_id', $attr)) {
        if ($attr['quiz_type_id'] != $old->quiz_type_id) {
          $new_type_sections = $this->type_list_section($attr["quiz_type_id"]);
          $old_type_sections = $this->type_list_section($old->quiz_type_id);
          $match_sections = array();
          $match_index = 0;
          foreach ($new_type_sections as $new_section) {
            foreach ($old_type_sections as $old_section) {
              if ($new_section->section_type == $old_section->section_type) {
                $match_sections[$match_index]["new"] = $new_section->id;
                $match_sections[$match_index]["old"] = $old_section->id;
                $match_index++;
                break;
              }
            }
          }
          $this->_question_section_update($id, $match_sections);
        }
      ...
      if (array_key_exists('target_id', $attr)) {
        $this->db->set('target_id', $attr['target_id']);
      }
  ...
  /**
   * Update all question section id when quiz type is changed.
   * @param int   $quiz_id        row id in quiz table
   * @param array $match_sections pairs of new and old type section id
   */
  private function _question_section_update($quiz_id = 0, $match_sections = array()) {
    foreach ($match_sections as $match) {
      $this->db->update($this->tables["question"], array("section_id" => $match["new"]), array("section_id" => $match["old"], "quiz_id" => $quiz_id));
    }
  }
  ...
  /**
   * Get all trainings' target
   * @param  bool   $all_sites determind site limitation
   * @return string json encoded found trainings'target
   */
  function get_all_training_s_target($all_sites = FALSE) {
    if (!$all_sites) {
      $site_id = $this->session->userdata("siteID");
      if ($site_id > 1) {
        $this->db->where("site_id", $site_id);
    ...
    $result = $this->db->select("id, target_id")->distinct()->get($this->tables["training"])->result();
    if (empty($result)) {
      return '';
    }
    $target_of_trainings = array();
    foreach ($result as $row) {
      $target_of_trainings[$row->id] = $row->target_id;
    }
    return json_encode($target_of_trainings);
  }
}
