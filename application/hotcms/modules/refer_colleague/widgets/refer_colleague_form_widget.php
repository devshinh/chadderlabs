          ...
          if (array_key_exists('postback', $args)) {
            try {
              if ($args['postback']['near_location'] > 0) {
                ...
                $args['postback']['near_location_name'] = $location->name;
                $insurance_type = $item->name;
                $email = $location->main_email;
                $this->quote_model->email_request($email, $item, $args['postback'], $insurance_type);
              }else{
                //$this->load->model('location/location_model');
                //$location = $this->location_model->get_location_by_id((int)($args['postback']['near_location']));
                //$args['postback']['near_location_name'] = $location->name;
                $insurance_type = $item->name;
                $email = $this->config->item('default_email', 'quote');
                $this->quote_model->email_request($email, $item, $args['postback'], $insurance_type);                
              }
            }
            catch (Exception $e) {
