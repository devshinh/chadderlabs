...
class Order extends HotCMS_Controller {
  ...
  public function index($page_num = 1)
  {
    ...
    //die(var_dump)
    foreach($orders as $order){
        $order->user_info = $this->user_model->get_user_by_id($order->user_id);
    }
    $data['orders'] = $orders;
    // paginate
  ...
  public function edit($id, $page_num = 1)
  {
    ...
    $data['form']['shipping_phone'] = $this->_create_text_input('shipping_phone', $order->billing_phone, 50, 20, 'text');
    $data['form']['shipping_email'] = $this->_create_text_input('shipping_email', $order->billing_email, 50, 20, 'text');
    ...
    if(in_array($order->order_status,array('processing', 'completed'))){
        $data['form']['fedex_number'] = $this->_create_text_input('fedex_number', $order->fedex_number, 50, 20, 'text');
    }
...
*/
  ...
  {
    if ($order_id > 0) {
      $order = $this->order_model->order_load($order_id);
      $order_items = $this->order_model->order_load_items($order_id);
    }
    else {
        exit;
    }
