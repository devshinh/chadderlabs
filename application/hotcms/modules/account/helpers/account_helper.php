  ...
  function account_retailers_ajax($country_code)
  {
    ...
    foreach ($results as $v) {
        if($v->status == 1){
          ...
          $json['retailers'][$v->name] = array('name' =>$v->name.'(Pending)', 'id' => $v->id);
        }elseif($v->status == 2){
          $json['retailers'][$v->name] = array('name' =>$v->name .'(Closed)', 'id' => $v->id);
        }
    }
    return $json;
  }
...
if (!function_exists('account_add_badge')) {
    ...
    function account_add_badge($id_user, $badge_name) {
        
        $CI =& get_instance();
        $CI->load->config('user/user', TRUE);
        $CI->load->model('user/user_model');
        
        $CI->user_model->add_badge($id_user, $badge_name);
    }
}
/**
...
{
  function account_get_sites($user_id)
  {
    $CI =& get_instance();
    $CI->load->model('site/site_model');
    
    $sites = $CI->site_model->get_all_sites(true, false);
