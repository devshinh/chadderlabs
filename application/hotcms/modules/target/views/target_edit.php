<div>
  <?=form_open($module_url."/edit/".$target_id, array("id" => "targetEdit_".$target_id), $form["hidden"])?>
  <div class="row">
...
<?php if ($site_id > 1) { ?>
      <div class="row">
        ...
        <span><?=$form["site_options"][$form["selected_site"]]?></span>
        <input name="site_id" type="hidden" value="<?=$form["selected_site"]?>">
      </div>
<?php } else { ?>
      <div class="row">
        <?=form_label(lang("hotcms_for")." ".lang("hotcms_site").lang("hotcms__colon"), "site_id")?>
...
<script type="text/javascript">
  ...
  jQuery('document').ready(function() {
    ...
    jQuery('.orgFilters').change(function() {
      var start = new Date();
      console.log('before ajax @ ' + start.getTime());
      jQuery.ajax({
        data: jQuery('#targetEdit_<?=$target_id?>').serialize(),
        dataType: 'json',
        ...
        success: function(data) {
          ...
          console.log('received ' + (received.getTime() - start.getTime()) + ' ms later');
          treeObj = storeTree(data['stores_tree']);
          treeObj.bind('loaded.jstree', function() {
            var rendered = new Date();
            console.log('loaded ' + (rendered.getTime() - received.getTime()) + ' ms later');
          });
          treeObj.bind('check_node.jstree uncheck_node.jstree check_all.jstree', function() {
            var newChosenOrgs = '';
            var newChosenStores = '';
  ...
  // type somthing so I can upload this.
</script>