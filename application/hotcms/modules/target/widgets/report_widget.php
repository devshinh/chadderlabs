        ...
        if (is_array($args)) {
          ...
          if ($site_id > 0) {
            ...
            if ($page_number === 1) {
              ...
              $summary["reatilers"] = count($targeted_orgs);
              ...
              $summary["training_hours"] = number_format(($site_training_time / 3600), 2);
              $completed_quizzes = $this->site_model->get_completed_quizzes($site_id);
              $summary["quizzes_completed"] = count($completed_quizzes);
              $points_balance = $this->site_model->get_point_balance($site_id);
              ...
              foreach ($targeted_orgs as $targeted_org) {
                $orgs_breakdown[$targeted_org->id] = array(
                    "oraganization" => $targeted_org->name,
                    "locations" => 0,
                    "total_members" => 0,
                    "members_trained" => 0,
                    "quizzes_taken" => 0,
              ...
              $data["orgs_breakdown"] = array_2d_sort($orgs_breakdown, "oraganization");
              ...
              $targeted_members = $this->target_model->get_targeted_members($site_id, FASLE, $targeted_locations);
              foreach ($targeted_members as $targeted_location_member) {
                $locs_breakdown[$targeted_location_member->store_id]["total_members"]++;
                $locs_breakdown[$targeted_location_member->store_id]["training_time"] += $this->account_model->get_user_training_time($site_id, $targeted_location_member->user_id);
                if ($this->account_model->is_user_trained($targeted_location_member->user_id, $site_id)) {
                  $locs_breakdown[$targeted_location_member->store_id]["members_trained"]++;
                }
              }
              $completed_quizzes = $this->site_model->get_completed_quizzes($site_id);
              foreach ($completed_quizzes as $completed_quiz) {
              ...
              $data["locs_breakdown"] = array_2d_sort($locs_breakdown, "members_trained");
              ...
              $targeted_members = $this->target_model->get_targeted_members($site_id, FALSE, $report_loc);
              ...
              foreach ($users_breakdown as $id => $member) {
                if ($member["quizzes_taken"] > 0) {
                  $users_breakdown[$id]["avg_quiz_score"] = number_format(($member["total_quiz_score"] / $loc["quizzes_taken"]), 2);
                }
                $users_breakdown[$id]["training_time"] = number_format(($member["training_time"] / 3600), 2)." hrs";
                unset($users_breakdown[$id]["total_quiz_score"]);
                if (((float) $member["cost"]) > 0) {
                  $users_breakdown[$id]["cost"] = "$".number_format(((float) $member["cost"]), 2);
                }
              }
              $data["report_org"] = $report_org;
              $data["report_loc"] = $report_loc;
              $data["users_breakdown"] = array_2d_sort($users_breakdown, "training_time");
              ...
              foreach ($completed_quizzes as $completed_quiz) {
                $quizzes_breakdown[$completed_quiz->user_id] = array(
                ...
                if ($completed_quiz->points_earned > 0) {
                  $quizzes_breakdown[$taken_by_user->user_id]["points_awarded"] = $completed_quiz->points_earned;
                  $quizzes_breakdown[$taken_by_user->user_id]["cost"] = "$".number_format($this->site_model->get_withdrew_points_worth($site_id, $completed_quiz->create_timestamp), 2);
                }
              }
              $data["report_org"] = $report_org;
              $data["report_loc"] = $report_loc;
              $data["report_user"] = $report_user;
              $data["quizzes_breakdown"] = array_2d_sort($quizzes_breakdown, "training_time");
            }
