...
class Quote_model extends HotCMS_Model {
   ...
   */
  public function quote_list($live_only = TRUE, $page_num = 1, $per_page = 0)
  {
   ...
   * @return bool
   */
  public function email_request($email_address, $quote, $attr)
  {
    if (empty($attr) || !is_array($attr) || empty($quote) || $quote->id <=0 || $email_address == '') {
      return FALSE;
    }
    // general information
    $subject = 'B&W Insurance Quote Submission';
    $message = $subject . "\n";
    $message .=  'Sent at ' . date('Y-m-d H:i') . "\n";
    $message .= array_key_exists('title', $attr) ? 'Title: ' . $attr['title'] . "\n" : '';
    $message .= array_key_exists('first_name', $attr) ? 'First Name: ' . $attr['first_name'] . "\n" : '';
    $message .= array_key_exists('last_name', $attr) ? 'Last Name: ' . $attr['last_name'] . "\n" : '';
    $message .= array_key_exists('date_of_birth', $attr) ? 'Date of Birth: ' . $attr['date_of_birth'] . "\n" : '';
    $message .= array_key_exists('address_1', $attr) ? 'Address Line 1: ' . $attr['address_1'] . "\n" : '';
    $message .= array_key_exists('address_2', $attr) ? 'Address Line 2: ' . $attr['address_2'] . "\n" : '';
    ...
    $message .= array_key_exists('near_location_id', $attr) ? 'Nearest Location: ' . $attr['near_location_id'] . "\n" : '';
    $message .= "\n\n";
    // loop through questions
    foreach ($quote->questions as $q) {
      $field_name = 'fld_' . $q->id;
    ...
    return mail($email_address, $subject, $message);
  }
