...
class Location extends HotCMS_Controller {
  ...
  public function index($page_num = 1, $message = '') {
    ...
    $left_data['current'] = $this->location_model->get_all_locations(TRUE);
    ...
    self::loadBackendView($data, 'location/location_submenu', $left_data, 'location/location', $right_data);
  }
  ...
  public function edit($id, $message = '') {
    ...
    if ($this->form_validation->run()) {
      ...
      self::loadBackendView($data, 'location/location_submenu', $left_data, 'location/location_edit', $right_data);
    } else {
      $current_item = $this->location_model->get_location_by_id($id);
      $data['aCurrentItem'] = $current_item;
      ...
      $users = $this->location_model->get_users_for_location($id);
      if (empty($users)) {
        $right_data['users_msg'] = 'No user associated with this location.';
      } else {
       // $right_data['users'] = '<a onClick="return confirmDelete()" href="/hotcms/location/delete_users/' . $id . '">Delete all users</a>';
  ...
  private function set_edit_form($currentItem) {
    ...
    $data['page_location_description'] = array(
        ...
        'value' => set_value('page_location_description', $currentItem->page_location_description),
        'rows' => '10',
        'cols' => '30',
        'class' => 'textarea'
    );
    $data['page_location_services'] = array(
        ...
        'value' => set_value('page_location_services', $currentItem->page_location_services),
        'rows' => '10',
        'cols' => '30',
        'class' => 'textarea'
    );
