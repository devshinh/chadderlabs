...
class CmsTraining extends CmsTrainingBase {
  ...
  public function load($live_only = TRUE, $current_site_only = TRUE)
  {
    ...
    $row = $this->ci->training_model->training_load($this->id, $this->slug, $live_only, $current_site_only);
    if ($row) {
  ...
  protected function get_draft()
  {
    ...
    if (empty($this->_draft) || $this->_draft->id == 0) {
      $this->ci->training_model->draft_insert($this);
   ...
   * @return bool
   */
  protected function get_has_quiz()
  {
   ...
   * @return mixed
   */
  public static function list_training($filters = FALSE, $live_only = TRUE, $page_num = 1, $per_page = 0)
  {
    $CI = & get_instance();
    return $CI->training_model->training_list($filters, $live_only, $page_num, $per_page);
  }
   ...
   * @return int
   */
  public static function count_all($filters = FALSE, $live_only = TRUE)
  {
    $CI = & get_instance();
    return $CI->training_model->training_count($filters, $live_only);
  }
  ...
  public function load($id, $rid = 0)
  {
    if ($id > 0) {
      ...
      if ($row) {
        ...
        $this->assets = unserialize($row->assets);
