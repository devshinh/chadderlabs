...
class Product_list_widget_admin extends Widget {
  ...
  public function run( $args=array() )
  {
    ...
    if (array_key_exists('postback', $args)) {
      ...
      if ($product_id > 0) {
        ...
        //if ($new_asset_id > 0) {
        //  $item_id = $this->product_model->insert_item($product_id, $new_asset_id);
        //}
        $deletes = $args['delete'];
        $ids = $args['id'];
        $links = $args['link'];
        ...
        if (is_array($deletes) && count($deletes) > 0) {
          ...
          foreach ($deletes as $id) {
            if ($id > 0) {
              $item = $this->product_model->get_item($id);
        ...
        if (is_array($ids) && count($ids) > 0) {
          foreach ($ids as $id) {
            ...
            $this->product_model->update_item($id, $links[$id], $titles[$id], $sequences[$id]);
          }
        }
    ...
    // load widget view
    return $this->render('product_list_admin', $data);
  }
